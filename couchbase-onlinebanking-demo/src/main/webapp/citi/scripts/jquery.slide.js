...
(function($){
	$.fn.Slide=function(options){
		...
		if(opts.effect=="scroolY"||opts.effect=="scroolTxt"){
			...
			ContentBox.css("width", 1000);//ContentBoxNum*slideW);
			slideWH=slideW;
		}else if(opts.effect=="fade"){
			ContentBox.children().first().css("z-index","1");
		}
		
		//$("#chip_prev").addClass("prev_empty");
		...
		return this.each(function() {
			...
			clickNext.click(function(event){
				...
				nNum++;
				if(4+nNum==ContentBoxNum){
				    $("#chip_next").addClass("next_empty");
				    $("#chip_prev").removeClass("prev_empty");
				}
				event.preventDefault();
			});
			clickPrev.click(function(event){
			    //debugger;
			    if(ContentBox.find("li:first").find(".slidebox").attr("info")=="0"){
				  return;
				}
				...
				for(var i=0;i<opts.steps;i++){
	          	...
	          	nNum--;
	          	if(nNum<=0){
				        $("#chip_next").removeClass("next_empty");
				        $("#chip_prev").addClass("prev_empty");
